...
vtime = otime[0:744]
bstars = np.array([stars[0:744], stars[0:744], stars[0:744]])
bdiff = np.array([stars[0:734], stars[0:734], stars[0:734]])
btime = otime[0:734]
rstars = np.array([stars[0:744], stars[0:744], stars[0:744]])
...
for i in range(0,d):
    if (i%3==0):
        if (ifilter[i]=="V"):
            ...
            if (ifilter[i]=="B"):
                bstars[0,a] = stars[i]
                bstars[1,a] = stars[i+1]
                bstars[2,a] = stars[i+2] 
                ...
                rstars[0,a] = stars[i]
                rstars[1,a] = stars[i+1]
                rstars[2,a] = stars[i+2] 
...
plt.legend()
plt.xlim(0, 1)
plt.title("Light Curve for SW Lac from Gordon" )
plt.xlabel(r'Phase')
plt.ylabel(r'Magnitude difference between SW Lac and *comparison star*')
...
gbs1, gbs1err, gbs2, gbs2err, gbs3, gbs3err, gbHJD, gb21, gb31, gb32, gbepoch, gberr32, gberr21, gberr31, gbphase = np.genfromtxt('datagb.txt', dtype='float', delimiter=None, skip_header=1, names=None, unpack=True)
gvs1, gvs1err, gvs2, gvs2err, gvs3, gvs3err, gbHJD, gv21, gv31, gv32, gvepoch, gverr32, gverr21, gverr31, gvphase = np.genfromtxt('datagv.txt', dtype='float', delimiter=None, skip_header=1, names=None, unpack=True)
...
k=k+1
plt.figure(k)
#plt.errorbar(vtime, unp.nominal_values(np.array(vdiff[0,:])), yerr=unp.std_devs(vdiff[0,:]), xerr=None, fmt='gx', linewidth=0.05, label='V-Fliter')
#plt.errorbar(btime, unp.nominal_values(np.array(bdiff[1,:])), yerr=unp.std_devs(bdiff[1,:]), xerr=None, fmt='bx', linewidth=0.05, label='B-Fliter')
#plt.errorbar(rtime, unp.nominal_values(np.array(rdiff[2,:])), yerr=unp.std_devs(rdiff[2,:]), xerr=None, fmt='rx', linewidth=0.05, label='R-Fliter')
plt.errorbar(gbphase, gb21, yerr=gberr21, xerr=None, fmt='g.', linewidth=0.05, label='V-Fliter', markersize=1)
plt.errorbar(gvphase, gv21, yerr=gverr21, xerr=None, fmt='bx', linewidth=0.05, label='B-Fliter', markersize=1)
plt.errorbar(grphase, gr31, yerr=grerr31, xerr=None, fmt='r+', linewidth=0.05, label='R-Fliter', markersize=1)
plt.legend()
plt.title("Light Curve for SW Lac from Gordon" )
plt.xlabel(r'')
plt.ylabel(r'Magnitude difference between SW Lac and comparison star')
...
print(hdtime)
phase= [((x - 2400000 - epoch)%period)/period for x in htime]
